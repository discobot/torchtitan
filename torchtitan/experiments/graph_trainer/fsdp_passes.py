...
from torchtitan.experiments.graph_trainer.fake_pg_pass import _register_derived_pg
...
def reassign_collective_pgs_pass(
    ...
    for source, derived in pg_mapping.items():
        _register_derived_pg(source, derived)
...
def schedule_fsdp_comms_to_dense_regions_pass(
    ...
    *,
    ...
    def _sink_output_only_wait_closure(wait: fx.Node) -> tuple[bool, str | None]:
        """Sink an RS wait and pure output-unpack users to the graph tail."""
        ...
        while work:
            ...
            if n in closure:
                continue
            if n is not wait and n.target not in _FSDP_WAIT_OUTPUT_OPS:
                return (
                    False,
                    f"{wait.name} has non-output user {n.name} ({n.target})",
                )
            closure.add(n)
            for user in n.users:
